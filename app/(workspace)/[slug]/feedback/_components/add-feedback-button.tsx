...
// switching Newest/Most Voted), which isn't a plain element yet and made the
// button intermittently render unstyled or (worse, before the Button-side
// fix) wrapped in a stray extra <button>. Constructing the Link client-side
// sidesteps that entirely — it's a normal, already-resolved element on every
// render. Shared by page.tsx and loading.tsx so both stay pixel-identical.
export function AddFeedbackButton({ slug }: { slug: string }) {
  return (
    <Button asChild>
